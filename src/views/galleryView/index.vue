...
const fetchRanking = async () => {
  const res = await getRankingList({ page, pageSize, character_key: "agst" });
  if (res.success) {
...
/* 画廊容器整体背景改为海感光斑 + 深海底 */
.gallery-container {
  background: radial-gradient(circle at center, #071018 0%, $bg 100%);
  color: $text;
  min-height: 100vh;
  padding-bottom: 60px;
  padding-top: 20px;
      ...
      .card {
        ...
        .card-inner {
          ...
          .overlay {
            ...
            width: 100%;
            padding: 12px 0;
            background: linear-gradient(transparent, rgba(2, 6, 10, 0.8));
            text-align: center;
            opacity: 0;
  ...
  /* 右侧排行面板：奥古斯塔风格（清冷冰蓝 + 圣光金 + 深海暗蓝），已写实颜色（无变量）*/
  .ranking-panel {
    width: 220px;
    padding: 16px;
    position: fixed;
    ...
    .panel-header {
      ...
      .ranking-title {
        ...
        letter-spacing: 0.6px;
        text-shadow: 0 6px 18px rgba(6, 4, 8, 0.48);
      }
      ...
      .toggle-icon {
        ...
        box-shadow: 0 6px 14px rgba(4, 4, 8, 0.48),
          inset 0 1px 0 rgba(116, 199, 255, 0.02);
    ...
    .ranking-item {
      ...
      &.rank-3 {
        ...
        background: linear-gradient(
          ...
          $accent-1 0%,
          $accent-2 42%,
          /* 代替 mix($accent-1, $accent-2, 30%) */ rgba(6, 10, 18, 0.96) 100%
        );
        color: #efeefe;
